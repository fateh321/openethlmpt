...
for priv_key in open(genesis_secrets[1:-1], "r").readlines():
    ...
    converted_addr = Web3.toChecksumAddress(addr)
    balance_string += f'{priv_key},{converted_addr}'
    # print(priv_key "," converted_addr)
    account_i += 1
    if account_i == num:
        break
    else:
